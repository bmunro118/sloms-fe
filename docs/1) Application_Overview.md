...
55. The `ReadOnly` role no longer has the Customers nav item in its navigation policy. ReadOnly accounts (used as initial customer-facing accounts) see: Dashboard, Orders, Documents, Price List, and Account — but not Customers.
